# 🏛️ Projeto MVC - Arquitetura de Computadores

Este repositório contém o código-fonte de um projeto desenvolvido do zero durante as aulas da disciplina de Arquitetura de Computadores, do curso de Análise e Desenvolvimento de Sistemas. O objetivo principal é aplicar e compreender na prática o padrão de arquitetura **MVC (Model-View-Controller)**.

## 🚀 Tecnologias Utilizadas

* **JavaScript (Node.js):** Ambiente de execução e linguagem principal.
* **NPM:** Gerenciador de pacotes para as dependências do projeto.

## 📁 Estrutura da Arquitetura MVC

O projeto foi organizado separando as responsabilidades para facilitar a manutenção e escalabilidade do código:

* **Model (Modelo):** Camada responsável pela lógica de negócios e interação com o banco de dados. Gerencia os dados da aplicação.
* **View (Visão):** Camada responsável pela interface com o usuário. É o que o cliente vê e interage.
* **Controller (Controlador):** O "cérebro" que faz a ponte entre o Model e a View. Recebe as requisições do usuário, processa os dados no Model e envia a resposta para a View.

## ⚙️ Como rodar o projeto

1.  Clone este repositório:
    ```bash
    git clone [https://github.com/DarioN370/nome-do-seu-repositorio.git](https://github.com/DarioN370/nome-do-seu-repositorio.git)
    ```
2.  Acesse a pasta do projeto:
    ```bash
    cd nome-do-seu-repositorio
    ```
3.  Instale as dependências:
    ```bash
    npm install
    ```
4.  Inicie a aplicação:
    ```bash
    npm start
    ```

---
👤 **Desenvolvido por Neto**
🔗 [GitHub - DarioN370](https://github.com/DarioN370)